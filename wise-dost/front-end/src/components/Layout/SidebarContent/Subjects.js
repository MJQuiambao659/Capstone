import React, { useState, useEffect } from "react";
import {
  Typography,
  Paper,
  useTheme,
  useMediaQuery,
  Button,
} from "@mui/material";
import { DataGrid } from "@mui/x-data-grid";
import axios from "axios";
import MenuBookIcon from "@mui/icons-material/MenuBook";
import EditIcon from "@mui/icons-material/Edit";
import DeleteIcon from "@mui/icons-material/Delete";
import { GridAddIcon } from "@mui/x-data-grid";
import DeleteModal from "../../Forms/SideBar Forms/DeleteModal";
import AddModal from "../../Forms/SideBar Forms/AddModal";
import EditModal from "../../Forms/SideBar Forms/EditModal";

const Subjects = () => {
  const theme = useTheme(); // Access the theme object
  const isSmallScreen = useMediaQuery(theme.breakpoints.down("sm"));
  const [subjectData, setSubjectData] = useState([]);
  const [newSubject, setNewSubject] = useState({
    subject_code: "",
    subject_name: "",
  });

  const buttonEditStyle = {
    display: "flex",
    alignItems: "center",
    border: "none",
    padding: 0,
    cursor: "pointer",
    backgroundColor: "transparent",
  };

  const buttonDeleteStyle = {
    display: "flex",
    alignItems: "center",
    border: "none",
    padding: 0,
    cursor: "pointer",
    backgroundColor: "transparent",
  };

  const [isAddDialogOpen, setAddDialogOpen] = useState(false);

  const [subjectToDelete, setSubjectToDelete] = useState(null);
  const [isDeleteDialogOpen, setDeleteDialogOpen] = useState(false);

  const [isEditDialogOpen, setEditDialogOpen] = useState(false);
  const [subjectToEdit, setSubjectToEdit] = useState(null);

  const handleAdd = (newSubject) => {
    axios
      .post("http://localhost:3001/subjects/add", newSubject)
      .then((response) => {
        // Update the programData state with the newly added program
        setSubjectData([...subjectData, response.data]);

        // Reset the newprogram state to clear the form
        setNewSubject({
          subject_code: "",
          subject_name: "",
        });

        // Close the add dialog
        setAddDialogOpen(false);
      })
      .catch(function (error) {
        console.error(error);
      });
  };

  const handleEdit = (row) => {
    // Open the Edit dialog with the selected program
    setEditDialogOpen(true);
    setSubjectToEdit(row);
  };

  const handleSaveEdit = (editedSubject) => {
    // Send a PUT request to update the program on the server
    axios
      .put(
        `http://localhost:3001/programs/edit/${editedSubject.subject_id}`,
        editedSubject
      )
      .then(() => {
        // Update the programData state to reflect the changes
        setSubjectData((prevData) =>
          prevData.map((subject) =>
            subject.subject_id === editedSubject.subject_id
              ? editedSubject
              : subject
          )
        );
        // Close the edit dialog
        setEditDialogOpen(false);
      })
      .catch(function (error) {
        console.error(error);
        // Handle the error as needed
      });
  };

  const handleDelete = (subjectId) => {
    const subject = subjectData.find(
      (subject) => subject.subject_id === subjectId
    );
    setSubjectToDelete(subject);
    setDeleteDialogOpen(true);
  };

  const handleConfirmDelete = () => {
    // Send a DELETE request to delete the program on the server
    axios
      .delete(
        `http://localhost:3001/subjects/delete/${subjectToDelete.subject_id}`
      )
      .then(() => {
        // Update the programData state to remove the deleted program
        setSubjectData((prevData) =>
          prevData.filter(
            (subject) => subject.subject_id !== subjectToDelete.subject_id
          )
        );
        // Close the confirmation dialog
        setDeleteDialogOpen(false);
      })
      .catch(function (error) {
        console.error(error);
        // Handle the error as needed
      });
  };

  useEffect(() => {
    axios
      .get("http://localhost:3001/subjects/data")
      .then((response) => {
        setSubjectData(response.data);
      })
      .catch(function (error) {
        console.error(error);
      });
  }, []);
  const columns = [
    { field: "subject_id", headerName: "ID", flex: 1 },
    {
      field: "subject_code",
      headerName: "Subject Code",
      flex: 2,
    },
    { field: "subject_name", headerName: "Subject Name", flex: 3 },
    {
      field: "actions",
      headerName: "Actions",
      width: 250,
      renderCell: (params) => (
        <div
          style={{
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
            gap: "8px",
          }}
        >
          <button
            style={buttonEditStyle}
            onClick={() => handleEdit(params.row)}
          >
            <EditIcon style={{ color: "green" }} />{" "}
            <span style={{ color: "green", fontWeight: "bold" }}>Edit</span>
          </button>
          <button
            style={buttonDeleteStyle}
            onClick={() => handleDelete(params.row.subject_id)}
          >
            <DeleteIcon style={{ color: "red" }} />{" "}
            <span style={{ color: "red", fontWeight: "bold" }}>Delete</span>
          </button>
        </div>
      ),
    },
  ];

  return (
    <Paper sx={{ padding: 2, width: isSmallScreen ? "80%" : "100%" }}>
      <Typography
        variant="h6"
        gutterBottom
        style={{
          display: "flex",
          alignItems: "center",
          borderBottom: "2px solid gray",
          marginBottom: "20px",
        }}
      >
        <MenuBookIcon style={{ marginRight: "8px" }} /> Subjects
        <Typography
          variant="body2"
          style={{ marginLeft: "8px", color: "#777" }}
        >
          manage available Subjects here...
        </Typography>
      </Typography>
      <Button
        startIcon={<GridAddIcon />}
        color="primary"
        onClick={() => setAddDialogOpen(true)}
        style={{ fontWeight: "bold" }}
      >
        Add Subject
      </Button>
      <div style={{ height: 400, width: "100%" }}>
        <DataGrid
          rows={subjectData}
          columns={columns}
          pageSize={5}
          getRowId={(row) => row.subject_id}
        />
      </div>

      <AddModal
        open={isAddDialogOpen}
        onClose={() => setAddDialogOpen(false)}
        onAdd={handleAdd}
        title="Add Subjects"
        fields={[
          { name: "subject_code", label: "Subject Code" },
          { name: "subject_name", label: "Subject Name" },
        ]}
        initialData={{ subject_code: "", subject_name: "" }}
      />

      <EditModal
        open={isEditDialogOpen}
        onClose={() => setEditDialogOpen(false)}
        onSave={handleSaveEdit}
        title="Edit Data"
        content={[
          { name: "subject_code", label: "Subject Code" },
          { name: "subject_name", label: "Subject Name" },
        ]}
        initialData={subjectToEdit}
      />

      <DeleteModal
        open={isDeleteDialogOpen}
        onClose={() => setDeleteDialogOpen(false)}
        onConfirm={handleConfirmDelete}
        title="Delete Subject"
        content={
          <div>
            <span>Are you sure you want to delete the program: </span>
            <span style={{ fontWeight: "bold" }}>
              {subjectToDelete?.subject_name}
            </span>
            <span>?</span>
          </div>
        }
      />
    </Paper>
  );
};

export default Subjects;
